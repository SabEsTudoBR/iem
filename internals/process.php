<?php
/**
 * This file provides licensing "process" for IEM 5.5
 *
...
function get_available_user_count()
{
	...
	$allowed_normal_users = intval($lice->{$command_get_user}());
	$allowed_trial_users = intval($lice->{$command_get_trial_user}());

	$available_users = array(
		'normal' => $allowed_normal_users - $current_users['normal'],
		'trial' => $allowed_trial_users - $current_users['trial']
	);
...
function ssk23twgezm2()
{
	...
	$agencyid = $lice->GetAgencyID();
	$allowed_normal_users = intval($lice->GetUsers());
	$allowed_trial_users = (empty($agencyid) ? 0 : intval($lice->GetTrialAccountLimit()));
	...
	$admin_user = $db->FetchOne("SELECT COUNT(1) AS count FROM [|PREFIX|]users WHERE admintype = 'a'", 'count');
	if ($admin_user === false) {
		return false;
	}
	...
	// You can create both "normal" and "trial" user
	if ($available_normal_user > 0 && $available_trial_user > 0) {
 ...
 */
function check_user_dir($uid, $upgrade)
{
